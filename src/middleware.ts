...
export default async function middleware(req: NextRequest) {
  ...
  if (pathname.startsWith("/_next") ||pathname.startsWith("/api") ||pathname.includes("favicon.ico") ||pathname.includes("assets")) {
    return NextResponse.next();
  }
  ...
  try {
    const res = await fetch(
      new URL(`/api/${encodeURIComponent(sessionID)}/user/load`, req.url),
      { cache: "no-store" }
    );

    if (res.ok) {
      userInfo = (await res.json()) as User;
    }
  } catch {
    return NextResponse.next();
  }
  ...
  if (isAdminRoute && !userInfo.admin) {
    return NextResponse.redirect(new URL(HOME_ROUTE, req.url));
  }

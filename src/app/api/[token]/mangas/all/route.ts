import { getMangasFollowed } from "@/bdd/requests/manga.request";
import { getUserByToken } from "@/bdd/requests/user.request";
import { ServerError } from "@/lib/api/response/server.response";

/**
 * Route : /api/[token]/mangas/all
 * METHOD : GET
 * 
...
export async function GET(req: Request, context: any): Promise<Response> {
    try {
        ...
        if (userRequest) {
            ...
            const data = await getMangasFollowed(limit, page, userRequest.id, isWaitList);
        ...
        return ServerError('GET : /api/[token]/mangas/all', err);
    }
}
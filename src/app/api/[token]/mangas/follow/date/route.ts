import { updateDateFollowManga } from "@/bdd/requests/manga.request";
import { getUserByToken } from "@/bdd/requests/user.request";
import { ServerError } from "@/lib/api/response/server.response";

/**
 * Route : /api/[token]/mangas/follow/date
 * METHOD : POST
 * 
 * Route de l'api modifiant la date de follow d'un manga.
 * 
...
export async function POST(req: Request, context: any): Promise<Response> {
    try {
        ...
        if(userRequest){
            ...
            const { mangaId, newDate } = requestBody;
            if(mangaId && newDate){
                const response = await updateDateFollowManga(userRequest.id, mangaId, newDate);
                if(response){
        ...
        return ServerError('/api/[token]/mangas/follow/date', err);
    }
}